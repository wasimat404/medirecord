...
export function distOutside(v: number, r: any) {
...
export function bmiOf(profile: any): string | null {
  return profile?.height_cm && profile?.weight_kg
    ? (profile.weight_kg / Math.pow(profile.height_cm / 100, 2)).toFixed(1)
    : null;
}

export function ageOf(profile: any): number | null {
...
export function buildSeries(records: any[]) {
  const recs = records || [];
  const points = recs.flatMap((r: any) =>
    (r.data_points || []).map((d: any) => ({ ...d, date: r.report_date || r.created_at }))
  );
...
export function buildMetrics(series: Record<string, any[]>): Metric[] {
  return Object.entries(series)
    ...
    .map(([test, arr]) => {
      ...
      if (arr[0].dual) {
        const all = arr.flatMap((a) => [a.sys, a.dia]);
      ...
      if (range) {
        const d0 = distOutside(arr[0].v, range), d1 = distOutside(last.v, range);
        ...
        const dl = last.v - arr[arr.length - 2].v;
...
export function buildVitals(series: Record<string, any[]>, profile: any): Vital[] {
  ...
  const vital = (re: RegExp, label: string): Vital | null => {
    ...
    if (last.dual) {
      return { label, value: `${last.sys}/${last.dia}`, unit: "mmHg", spark: mini(arr.map((a: any) => a.sys)), ok: last.sys < 130 && last.dia < 85 };
    }
...
export function buildInsights(metrics: Metric[], points: any[]): Insight[] {
...
export function buildHealth(records: any[], profile: any) {
